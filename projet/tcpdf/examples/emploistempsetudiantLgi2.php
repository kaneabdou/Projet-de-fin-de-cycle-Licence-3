...
";         
              ...
              for ($i=0; $i < count($arrayHour); $i++) {
                ...
                for ($j=0; $j < count($arrayDay); $j++) {
                    ...
                    $reqEmploi = $bdd->prepare("SELECT * FROM emploitemps  INNER JOIN cours ON cours.filiere = :filiere INNER JOIN salles ON salles.numSalle = emploitemps.idSalle WHERE emploitemps.idCour = cours.idCours AND idJour=:idJour AND heure=:heure AND cours.niveau= 2 ");
...
$html = <<<EOD
<h1>EMPLOIS DU TEMPS LGI 2ieme ANNEE année.</h1>
$tableHtml
EOD;
